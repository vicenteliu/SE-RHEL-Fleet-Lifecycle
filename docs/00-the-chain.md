...
| 5 | A playbook on one laptop → automation with an owner, a record and a permission model | job templates · credentials · RBAC · schedules · execution environments | Ansible Automation Platform | AWX (controller upstream) · ansible-core · Galaxy · `ansible-runner` · playbooks from a control node (the author's 🔨) | 🔨 lab — a patch playbook against the `dev` host, `--check` then real, state read back from both VMs (run 2026-09-15) · ⛔ controller · playbooks 🔨 (prior) | [5](../phases/5-automation/) |
...
Phases 0, 1, 4 and the playbook half of 5 ran the day a developer subscription was registered on the tier.

...
## Phase 0 — subscription and support (⛔ on the tier; rows kept because the boundary is the point)

| # | Responsibility | Human decides | Agent executes | How | How you know it worked | Model | Tested on | Status | Run |
|---|---|---|---|---|---|---|---|---|---|
| 0.1 | Register a host with an activation key | Which key — the key is the environment and the content the host will live with | The registration and the read-back | `subscription-manager register --activationkey` · `status` · `repos --list-enabled` | `Registered`; only the key's repositories enabled | | | ⛔ tier — runnable the day an account exists | |
